function mss($, _){
    ...
    function MSSTree(layer){
        ...
        this.sign = function(seed, li, message){
            ...
            var auths = [];
            var authPos = ((li % 2 == 0)?li+1:li-1);

            for(var i=0; i<layer; i++){
                auths.push(authPos);
                if(authPos % 2 == 1) authPos -= 1;
                authPos = Math.floor(authPos / 2);
            };

            // feed the tree to get root, and by the way get auth.
            var freshNode;
            for(var i=0; i<capacity; i++){
                freshNode = treehash.feed(i);
                if(!isNaN(auths[freshNode[1]])){
                    if(0 == auths[freshNode[1]]){
                        auths[freshNode[1]] = freshNode[0];
                    } else {
                        auths[freshNode[1]] -= 1;
                    };
                };
