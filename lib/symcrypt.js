...
//                    console.log(reversedSeq[i] + ' is taking over.', keys[i].slice(0,32).toString('hex'));
    ...
    return new function(){
        ...
        this.encryptEncodedCompressed = function(key, plaintext, rueckruf){
            $.nodejs.async.waterfall([
                function(callback){
                    $.nodejs.zlib.deflateRaw(plaintext, callback);
                },

                function(compressed, callback){
                    self.encryptEncoded(key, compressed, callback);
                },
            ], function(err, result){
                rueckruf(err, result);
            });
        ...
        this.decryptEncodedCompressed = function(key, ciphertext, rueckruf){
            $.nodejs.async.waterfall([
                function(callback){
                    self.decryptEncoded(key, ciphertext, callback);
                },
                function(compressed, callback){
                    $.nodejs.zlib.inflateRaw(compressed, callback);
                },
            ], function(err, result){
                rueckruf(err, result);
            });
