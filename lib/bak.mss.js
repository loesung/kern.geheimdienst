        ...
        function iterateHashInit(privateKey, i){
            return algorithm(
                $.nodejs.buffer.Buffer.concat([
                    privateKey,
                    new $.nodejs.buffer.Buffer((i).toString()),
                ])
            );
        };
...
//            console.log('Leaf #', phi, hashResult.toString('hex'));
            return [
                hashResult,
                0
            ];
...
//            console.log('li', li);
    ...
    /* export only testing */
    this.sign = function(privateKey, message){
        var singleTree = new MSSTree(5);
        return singleTree.sign(getTreeSeed(privateKey, 0), 0, message);
    };
